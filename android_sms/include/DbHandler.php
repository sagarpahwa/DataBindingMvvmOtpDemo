...
class DbHandler {
     ...
     * @param String $password User login password id
    ...
    public function createUser($name, $password, $mobile, $otp) {
        ...
        if (!$this->isUserExists($mobile)) {
            ...
            $stmt->bind_param("ssss", $name, $password, $mobile, $api_key);
            ...
            // User with same password already existed in the db
    ...
    public function activateUser($otp) {
        $stmt = $this->conn->prepare("SELECT u.id, u.name, u.password, u.mobile, u.apikey, u.status, u.created_at FROM users u, sms_codes WHERE sms_codes.code = ? AND sms_codes.user_id = u.id");
        $stmt->bind_param("s", $otp);
 
        if ($stmt->execute()) {
            // $user = $stmt->get_result()->fetch_assoc();
            $stmt->bind_result($id, $name, $password, $mobile, $apikey, $status, $created_at);
